...
text(6, 0.98, 'EXP. 1', 'HorizontalAlignment', 'center','FontSize', 12)
...
ActionValues  = 0.5*ones(max(data(:,3)), max(data(:,1)) + 1);

for trialIndex = 1 : length(data)
    % Likelihood computation
    NegativelogLikelihood = NegativelogLikelihood - parameters(1) * ActionValues(data(trialIndex, 3), data(trialIndex, 1) + 1) + ...
        log(sum(exp(parameters(1) * ActionValues(data(trialIndex, 3), :))));
        
    % Action value update
    delta = data(trialIndex, 2) - ActionValues(data(trialIndex, 3), data(trialIndex, 1) + 1) ;
    ActionValues(data(trialIndex, 3), data(trialIndex, 1) + 1) = ActionValues(data(trialIndex, 3), data(trialIndex, 1) + 1) + ...
        parameters(2) * delta * (delta > 0) + parameters(3) * delta * (delta <= 0) ;
    
    % Action value normalization
    ActionValues(data(trialIndex, 3), 2 - data(trialIndex, 1)) = 1 - ActionValues(data(trialIndex, 3), data(trialIndex, 1) + 1) ;
end
